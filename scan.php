...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['waste_image'])) {
    ...
    if (move_uploaded_file($_FILES['waste_image']['tmp_name'], $target_file)) {
        ...
        // 🌟 自动化定位 Railway 内部高架桥网络中的 Python 后端服务
        ...
        // 🌟【核心隔离同步突破】：在这里将当前登录的真实 Session 用户名随图片一并封装！
        // 这样 Python 后端就能瞬间抓到是谁在扫描，完美实现多用户相互独立，分秒同步
        $post_data = array(
            'image' => $cFile,
            'username' => $username
        );
        ...
        // 🌟 注入超长响应超时，防止首次加载 YOLO 权重或内网波动超时
        ...
        // 🌟【业务解耦】：将数据库插入和垃圾桶更新的压力全部卸载给已经完美对齐时区的 Python 异步执行。
        // PHP 只需要负责接收 Python 的处理状态，并原封不动呈递给前端。
        if (isset($result_data['status']) && $result_data['status'] == 'success') {
            ...
            echo json_encode(['status' => 'error', 'message' => 'AI Core processing failed or database sync error inside backend.']);
        }
    } else {
        ob_end_clean();
        echo json_encode(['status' => 'error', 'message' => 'Failed to save uploaded image inside PHP server.']);
    }
    exit; 
}
?>
